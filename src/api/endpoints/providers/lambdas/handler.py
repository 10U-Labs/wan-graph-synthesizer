"""providers read endpoint: serve a cloud provider's regions from the S3 store.
...
A provider graph is regions only (no fiber), so it exposes vertices but no edges.
