"""The optimizer's design vocabulary: tiers, tuning, routing, and validation.

These types are private to the WAN designer -- the inputs script never touches them.
They build on the shared interchange types in ``wan_graph.model`` (``Vertex`` /
``PhysicalEdge`` and the geographic helpers), which travel between the two programs as
JSON; everything here is the optimizer's own in-memory representation.
"""
...
class ForcedLinks:
    ...
    ``wan_graph.model.edge_key`` pair; ``aggregation`` holds each ``aggregation-core``
