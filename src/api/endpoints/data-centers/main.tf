...
locals {
  function_name = module.common.lambda_handler_names.datacenters
  role_name     = "wan-graph-synthesizer-data-centers-lambda"
}
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 10
  memory_size      = 128
  description      = "Data-centers read endpoint: serve a provider's facilities from the store."
