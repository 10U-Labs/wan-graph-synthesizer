provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Stack      = "endpoints/data-centers/merge"
    }
  }
}
...
locals {
  function_name = module.common.lambda_handler_names.datacenters_merge
  role_name     = "wan-graph-synthesizer-data-centers-merge-lambda"
}
...
resource "aws_lambda_function" "handler" {
  ...
  timeout          = 30
  memory_size      = 256
  description      = "Data-centers merge endpoint: union every provider's facilities and serve it."
