"""The two-tier WAN design engine, run by the synthesizer endpoint.

Takes a JSON-loaded carrier graph plus a tenant's demand and produces a
validated two-tier (backbone / demand) design. The Fargate synthesizer
entrypoint composes the submodules directly (``dual_home`` -> ``apply_role_overrides``
-> ``synthesize_two_tier_design`` -> ``finalize``); this package exposes no
