...
# provider-data-center vertices are classified into provider/provider/provider families by
# name (checked most-specific first so "provider" is not caught by "provider"),
# then split regions by an "east"/"west" hint; a region with neither hint is
...
def region_layer(name: str) -> str | None:
    """Map a provider region's name to its ``<family>_<east|west>`` layer, or None."""
    lowered = name.lower()
