...
Carriers (PoPs + fiber) and providers (regions) are pushed as graphs; each customer's
installations, provider-region selection, and design config are pushed as its inputs.
...
def _customer_inputs(config: dict[str, Any]) -> tuple[list[Any], list[Any]]:
    """Split a config's vertices into (installations, provider-regions) by tenant."""
    ...
    for tenant, value in config.get("inputs", {}).get("vertices", {}).items():
        for raw in value if isinstance(value, list) else [value]:
            ...
            if path.stem in carriers:
                continue
            target = regions if tenant.lower() in provider_PROVIDERS else installs
...
def push_customers(api: str) -> None:
    """Push each customer's installations, provider regions, off-net seats, and config."""
