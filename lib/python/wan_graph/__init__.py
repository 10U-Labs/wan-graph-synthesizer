...
and the synthesizer (which reads them back): :mod:`wan_graph.model` holds the shared
vertex/edge dataclasses and geographic helpers, :mod:`wan_graph.codec` the input-graph
JSON codec. The synthesizer's own design vocabulary lives in ``wan_synthesizer.model``, not
