...
Shared machinery is tested in every workflow that imports it. A module under `lib/python/` has no workflow of its own and no single consumer, so its subtree under `test/lib/python/` runs in each workflow whose own tests import it — transitively, since `test_handler_contracts` imports `test_module_utils` and `test_s3_store_mock`, and `test_terraform_config` sits under `test_fixtures.aws` and `test_terraform_drift`. Each such workflow gates the module with a `--cov=lib/python/<module>` of its own, so a module arriving without tests fails rather than being carried by its neighbours' numbers, and lists `test/lib/python/<module>/**` in its `paths`.
