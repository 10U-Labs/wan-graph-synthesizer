...
The gate names all twelve jobs one by one, because the nine static-analysis checks and the three test jobs are independent of each other and any one of them can be the only red job in the run — see [every-check-is-its-own-job](every-check-is-its-own-job.md). The nine checks and `test-repo-libraries` are gated on nothing and run on every push, so all ten are `and`-ed in flat. The other two — `unit-tests` and `integration-tests` — skip on a `data/raw/`-only push, so they sit in an arm against `needs.concluding-testing-unnecessary.result == 'success'`, which is the only result that push leaves behind.

`test-repo-libraries` is read twice over, in the flat `and` and again down the chain, and the second reading is what covers the `data/raw/`-only push. `determining-testing` needs `test-repo-libraries`, so a red one skips it, skips both `concluding-*` gates, and leaves neither arm of `seeding`'s `if` a `success` to read. That is the only thing standing between a broken shared module and a PUT to the live API on the push where `unit-tests` and `integration-tests` both skip.
