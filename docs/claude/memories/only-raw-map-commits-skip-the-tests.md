...
`determining-testing` in `.github/workflows/seed.yml` reads the push diff and sets `testing-necessary=false` only when every changed path is under `data/raw/`. Those are the carriers' published network maps, the PDFs and images the cleaned `data/vertices/` and `data/edges/` files were transcribed from by hand, and no code opens them. Everything else tests, including a push that touches nothing but `etc/` or nothing but the cleaned data. What that decision skips is three jobs — `test-repo-libraries`, `unit-tests` and `integration-tests`. The nine static-analysis checks and `yamllint` are gated on nothing and run on every push, so a raw-map commit is still read by `pylint`, `mypy`, `jscpd`, `yamllint` and the three `assert-*` tools.
