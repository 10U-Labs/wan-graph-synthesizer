...
A name is a thing the reader can open: a path, a function, a file and line, an S3 object key, a workflow file, a config key, an endpoint and its method. `_settled` at line 91 of `test/api/endpoints/tenants/wan/post/post_deployment/integration/conftest.py` is a name. "The delivered-design layer" is not. When a phrase cannot be pasted into a search box and land somewhere, it is not naming anything, and the reader has to reconstruct what was meant from context that the writer had and they do not.
