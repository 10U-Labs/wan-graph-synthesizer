...
def test_every_pinned_city_can_carry_the_diversity_its_tenant_asks_for() -> None:
    ...
    Measured over the merged carriers today, nine of the thirty-five pinned cities sit
    exactly on the number their tenant asks for and the other twenty-six are above it --
    New York, NY under both DAF and DoW and F-35's Ashburn, VA, Hillsboro, OR and Seattle,
    WA, each with two paths out that share no city; Two-Node's Ashburn, VA and Salt Lake
    City, UT, joined by two paths sharing nothing but their two ends, 1,843.1 and 2,041.1
    miles against 1,815.7 direct; and Yurop's Ashburn, VA and Stuttgart, Germany, which ask
    for one path apiece and have one. The bound counts only paths to the tenant's other
    pins, so the real run, which seats more sites and adds fiber segments, can only do
    better.
