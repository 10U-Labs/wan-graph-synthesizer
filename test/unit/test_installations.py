...
def _realize(*extra: Vertex) -> RealizedInstallations:
    """Realize installations over the three PoPs plus the given extra vertices."""
    return realize_installations([*_pops(), *extra], {})


def test_realize_installations_seats_a_twin() -> None:
    """A justified installation near carrier PoPs gets a co-located facility twin."""
...
def test_realize_installations_ignores_unjustified_sites() -> None:
    """A not-justified installation never becomes a facility."""
    result = _realize(fixtures.access_vertex("plain", 0.0, 0.5))
    assert result.facility_ids == frozenset()


def test_realize_installations_skips_isolated_installation() -> None:
    """A justified site with no PoP in range stays demand-only (no twin)."""
    result = _realize(fixtures.justified_installation("remote", 0.0, 10.0))
    assert result.facility_ids == frozenset()


def test_realize_installations_collapses_colocated_sites() -> None:
    """Two justified sites at one location collapse to a single twin."""
    result = _realize(
