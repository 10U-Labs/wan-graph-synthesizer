"""Layer 1 (contracts): cross-file consistency for the data-centers/merge stack.

The data-centers/merge stack couples to the shared common module (whose outputs
its locals reference) and to the storage stack's remote state (where it reads the
...
def _stack_text() -> str:
    """Return the combined text of the merge stack's main.tf and iam.tf."""
    return ((MERGE_DIR / "main.tf").read_text(encoding="utf-8")
            + (MERGE_DIR / "iam.tf").read_text(encoding="utf-8"))
...
def test_lambda_arn_output_references_the_declared_handler() -> None:
    ...
    outputs = output_values(MERGE_DIR / "outputs.tf")
