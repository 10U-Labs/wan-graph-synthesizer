"""Layer 4 (state): the declared data-centers/merge state matches AWS reality.
...
def _has_existing_state() -> bool:
    ...
    return bool(get_state_resources(MERGE_DIR))
...
def test_no_orphaned_resources() -> None:
    ...
    assert not find_orphaned_resources(MERGE_DIR)
