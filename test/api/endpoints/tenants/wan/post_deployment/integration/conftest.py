"""Derived fixtures for the wan dispatcher post-deployment integration tier.

``lambda_config`` fetches the live dispatcher Lambda configuration once (the
``lambda_client`` and ``function_name`` fixtures come from parent conftests) so the
existence, configuration, and wiring layers share the call.
"""
