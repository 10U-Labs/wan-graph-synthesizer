"""Unit tests for the tenants endpoint stack's declared infrastructure.
...
def test_lambda_runtime_is_python313(tenants_main: dict[str, object]) -> None:
    """The handler Lambda runs on Python 3.13."""
    handler = _resource(tenants_main, "aws_lambda_function", "handler")
    assert handler["runtime"] == "python3.13"


def test_lambda_is_arm64(tenants_main: dict[str, object]) -> None:
    """The handler Lambda runs on ARM64 (Graviton)."""
    handler = _resource(tenants_main, "aws_lambda_function", "handler")
    assert handler["architectures"] == ["arm64"]


def test_lambda_entrypoint(tenants_main: dict[str, object]) -> None:
...
def test_log_group_retention_is_seven_days(tenants_main: dict[str, object]) -> None:
    """The handler's log group retains events for seven days."""
    log_group = _resource(tenants_main, "aws_cloudwatch_log_group", "handler")
    assert log_group["retention_in_days"] == 7


def test_iam_role_is_declared(tenants_iam: dict[str, object]) -> None:
    """The Lambda execution role is declared."""
    assert find_resource(tenants_iam, "aws_iam_role", "lambda") is not None


def test_store_access_policy_is_declared(tenants_iam: dict[str, object]) -> None:
    """The store-access inline policy is declared on the role."""
    assert find_resource(tenants_iam, "aws_iam_role_policy", "store_access") is not None


def test_api_gateway_invoke_permission_is_declared(
        tenants_main: dict[str, object]) -> None:
    """API Gateway is granted permission to invoke the handler."""
    assert find_resource(tenants_main, "aws_lambda_permission", "api_gateway") is not None
