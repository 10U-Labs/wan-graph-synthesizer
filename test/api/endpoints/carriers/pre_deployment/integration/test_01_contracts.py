"""Layer 1 (contracts): cross-file consistency for the carriers stack.

The carriers stack couples to the shared common module (whose outputs its locals
reference) and to the storage stack's remote state (where it reads the store
bucket). These assert those couplings hold. No AWS calls.
"""
...
def _stack_text() -> str:
    """Return the combined text of the carriers stack's main.tf and iam.tf."""
    return ((CARRIERS_DIR / "main.tf").read_text(encoding="utf-8")
            + (CARRIERS_DIR / "iam.tf").read_text(encoding="utf-8"))
...
def test_lambda_arn_output_references_the_declared_handler() -> None:
    ...
    outputs = output_values(CARRIERS_DIR / "outputs.tf")
