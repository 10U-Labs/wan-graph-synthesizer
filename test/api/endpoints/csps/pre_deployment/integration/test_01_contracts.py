"""Layer 1 (contracts): cross-file consistency for the csps stack.

The csps stack couples to the shared common module (whose outputs its locals
...
def _stack_text() -> str:
    """Return the combined text of the csps stack's main.tf and iam.tf."""
    return ((CSPS_DIR / "main.tf").read_text(encoding="utf-8")
            + (CSPS_DIR / "iam.tf").read_text(encoding="utf-8"))
...
def test_lambda_arn_output_references_the_declared_handler() -> None:
    ...
    outputs = output_values(CSPS_DIR / "outputs.tf")
