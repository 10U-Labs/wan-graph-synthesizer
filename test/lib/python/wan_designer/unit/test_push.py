...
def _setup_providers(root: Path) -> None:
    """Write regions for aws only (provider and provider stay absent)."""
    aws = root / "vertices" / "providers" / "aws"
...
def test_push_providers_skips_providers_without_region_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """push_providers PUTs providers that have region files and skips the rest."""
    _setup_providers(tmp_path)
    requests = _capture_requests(monkeypatch)
    monkeypatch.setattr(seed,"DATA", tmp_path)
    seed.push_providers("http://api")
