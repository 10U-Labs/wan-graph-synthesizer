...
def _segment(kind: str, near: str, far: str, miles: float) -> dict[str, Any]:
    ...
    return {"source_id": near, "target_id": far, "distance_miles": miles, "edge_kind": kind}
...
def test_the_miles_ordered_are_the_carrier_fiber_the_network_runs_over() -> None:
    ...
    assert ordered_fiber_miles({"edges": _ORDERED}) == 360.75


def test_a_network_whose_edges_are_all_homings_ordered_no_fiber() -> None:
    """Sites homed into a backbone that holds no fiber of its own is no fiber ordered.

    A reader adding up every edge served would call this thirteen miles of network, which
    is thirteen miles nobody laid.
    """
    assert ordered_fiber_miles({"edges": _ORDERED[2:]}) == 0


def test_a_network_carrying_no_edges_ordered_no_fiber() -> None:
    """A tenant whose build has not landed published no fiber and reads as none."""
    assert ordered_fiber_miles({"edges": []}) == 0
