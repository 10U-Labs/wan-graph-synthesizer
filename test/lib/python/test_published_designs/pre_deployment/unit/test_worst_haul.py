...
a design is built here whose answer is known from the geometry rather than from the
...
from test_published_designs import worst_haul
...
def _design(*demand: dict[str, Any]) -> dict[str, Any]:
...
def test_the_worst_haul_is_the_farthest_site_from_the_node_nearest_it() -> None:
    ...
    assert worst_haul(design) == 138.2


def test_a_design_whose_every_site_is_exempt_reads_zero() -> None:
    ...
    own ``coverage_worst_haul`` gives for the same design and the branch a live network
    would only reach if an operator exempted every site it has.
    """
    assert worst_haul(_design(_site("oconus", 10.0, True))) == 0.0
