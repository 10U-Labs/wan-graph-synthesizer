...
class _Exceptions:
    """The exception names the checkers reach for, every one of them the same class."""

    def __getattr__(self, _name: str) -> type[BaseException]:
        """Answer any exception name with the class the refusals below raise."""
        return _Missing
...
class _Client:
    ...
    def __getattr__(self, _name: str) -> Callable[..., Any]:
        """Answer any method name with the canned answer."""
        return self._answer
...
def _asked(recorded: list[str], value: str) -> _Client:
